function [sb_solver] = solver_PDIS_LS(KKT,x_start,parameters_subs,warm_start)
tic
...
while true
    ...
    if ~isempty(index_pos_slack)
        s_safe =  theta*min(-slack(index_pos_slack)./Delta_slack(index_pos_slack)); % the mininum s that slack lambda remain non negative
        s = (min(s,s_safe));
        if ~g2o_style
